...
public:
  //! constructor
  TagInfo()
      : mDefaultValue(NULL), mMeshValue(NULL), mDefaultValueSize(0),
        mMeshValueSize(0), mDataSize(0), dataType(MB_TYPE_OPAQUE) {}

  //! constructor that takes all parameters
  TagInfo(const char *name, int size, DataType type, const void *default_value,
   ...
   */
  ErrorCode validate_lengths(Error *error_handler, const int *lengths,
  ...
  virtual ErrorCode get_data(const SequenceManager *seqman,
                             Error *error_handler, const EntityHandle *entities,
   ...
   */
  virtual ErrorCode get_data(const SequenceManager *seqman,
                             Error *error_handler, const Range &entities,
                             void *data) const = 0;
  ...
  virtual ErrorCode get_data(const SequenceManager *seqman,
                             Error *error_handler, const EntityHandle *entities,
                             size_t num_entities, const void **data_ptrs,
   ...
   */
  virtual ErrorCode set_data(SequenceManager *seqman, Error *error_handler,
                             const Range &entities, const void *data) = 0;
   ...
   *\param allocate    If true, space for this tag will be allocated, if not it
   *wont
   *
  ...
  virtual ErrorCode tag_iterate(SequenceManager *seqman, Error *error_handler,
                                Range::iterator &iter,
                                const Range::iterator &end, void *&data_ptr,
   ...
   */
  virtual ErrorCode
  find_entities_with_value(const SequenceManager *seqman, Error *error_handler,
